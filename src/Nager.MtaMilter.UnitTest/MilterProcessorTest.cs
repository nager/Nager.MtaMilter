...
namespace Nager.MtaMilter.UnitTest
{
    ...
    {
        private ILogger<MilterProcessor> GetLogger()
        {
            return LoggerHelper.GetLogger<MilterProcessor>().Object;
        }
        ...
        public void SMFIC_OPTNEG_Test()
        {
            ...
            var answerByte = milterProcessor.ProcessData(data);
        ...
        [TestMethod]
        public void SMFIC_CONNECT_Test1()
        {
            var logger = this.GetLogger();
            var milterProcessor = new MilterProcessor(logger);
            ...
            Assert.IsNotNull(answerByte);
            Assert.AreEqual(1, answerByte.Length);
            Assert.AreEqual(0x63, answerByte[0]);
        }

        [TestMethod]
        public void SMFIC_CONNECT_Test2()
        {
            var checkLogic = (ConnectMilterMessage connectMessage) =>
            {
                if (connectMessage.MailserverIpAddress == "172.17.0.1")
                {
                    return false;
                }
            ...
            var logger = this.GetLogger();
            var milterProcessor = new MilterProcessor(logger);
            milterProcessor.ConnectMessageReceived += checkLogic;
            ...
            Assert.IsNotNull(answerByte);
            Assert.AreEqual(1, answerByte.Length);
            Assert.AreEqual(0x61, answerByte[0]);
        }
